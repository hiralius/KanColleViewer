...
namespace Grabacr07.KanColleViewer.Models
{
	public static class ProductInfo
	{
		...
		private static readonly Lazy<string> versionLazy = new Lazy<string>(() => $"{Version.ToString(3)}{(IsBetaRelease ? " β" : "")}{(Version.Revision == 0 ? "" : " rev." + Version.Revision)}"
				+ (Attribute.IsDefined(assembly, typeof(AssemblyInformationalVersionAttribute)) ? $" {((AssemblyInformationalVersionAttribute)Attribute.GetCustomAttribute(assembly, typeof(AssemblyInformationalVersionAttribute))).InformationalVersion}" : ""));
